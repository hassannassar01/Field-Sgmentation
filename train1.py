        ...
        for j in range(args.batch_size):
            ...
            with tf.device('/cpu:0'):
                ...
                for i in range(2):
                    for j in range(3):
                        # Prep the data. Make sure the labels are in one-hot format
                        cropped_input_image = np.float32(input_image[i*args.crop_height:(i+1)*args.crop_height, j*args.crop_width:(j+1)*args.crop_width, :]) / 255.0
                        cropped_output_image = np.float32(helpers.one_hot_it(label=output_image[i*args.crop_height:(i+1)*args.crop_height, j*args.crop_width:(j+1)*args.crop_width, :], label_values=label_values))

                        # input_image_batch.append(np.expand_dims(cropped_input_image, axis=0))
                        # output_image_batch.append(np.expand_dims(cropped_output_image, axis=0))

                        # Do the training
                        _,current=sess.run([opt,loss],feed_dict={net_input:np.expand_dims(cropped_input_image, axis=0),net_output:np.expand_dims(cropped_output_image, axis=0)})
        ...
        for ind in val_indices:
            ...
            for i in range(2):
                for j in range(3):
                    # Prep the data. Make sure the labels are in one-hot format
                    cropped_input_image = np.float32(input_image[i*args.crop_height:(i+1)*args.crop_height, j*args.crop_width:(j+1)*args.crop_width, :])/255.0
                    cropped_output_image = sess.run(network,feed_dict={net_input:np.expand_dims(cropped_input_image, axis=0)})
                    full_image.append(cropped_output_image)

            output_image_h_1 = np.concatenate((full_image[0], full_image[1], full_image[2]), axis=2)
            output_image_h_2 = np.concatenate((full_image[3], full_image[4], full_image[5]), axis=2)
            output_image_v = np.concatenate((output_image_h_1, output_image_h_2), axis=1)
            output_image = np.array(output_image_v[0,:,:,:])
